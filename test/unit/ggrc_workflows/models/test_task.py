...
def task_factory(status, ret_is_templ, is_templ):
  ...
  Args:
      ...
      is_templ: Mocked Task().is_template property.
  ...
  is_templ.return_value = ret_is_templ
  test_task = task.Task()
...
class TestTask(unittest.TestCase):
  ...
  @patch.object(task.Task, 'is_template', new_callable=PropertyMock)
  @unpack
  @data((NOT_STARTED_STATUS, False), (IN_PROGRESS_STATUS, False),
        (FINISHED_STATUS, False), (TEMPLATE_STATUS, True))
  def test_validate_status_positive(self, status, ret_is_templ, is_templ):
    """Tests positive cases for Task().validate_status() method."""
    self.assertEqual(
        task_factory(status, ret_is_templ, is_templ).status, status)
  ...
  def test_validate_status_raises(self, status, ret_is_templ, err_msg,
    ...
    with self.assertRaises(ValueError) as err:
      self.assertIsNone(task_factory(status, ret_is_templ, is_templ).status)
